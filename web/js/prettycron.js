...
(function() {
  ...
  var getNext = function(cronspec, sixth) {
    // Update by Junhui for the time zone issue
    var date = new Date(getNextDate( cronspec, sixth )).toISOString();
    var result = moment(date).calendar();
    //if(result.indexOf('Today') != -1){return result};
    return date.replace('Z','').replace('T', ' ').trim().slice(0,-4);
  };
