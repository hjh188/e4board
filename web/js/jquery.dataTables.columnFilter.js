...
(function ($) {
    ...
    function fnCreateInput(regex, smart, bIsNumber) {
        ...
        var header_text = $(oApi.column(i).header()).text();
        //var input = $('<input type="text" class="search_init ' + sCSSClass + '" value="' + label + '"/>');
        var input = $('<div class="ui small filter input" style="width: 100%;"><input col="' + i + '"type="text" placeholder="Search ' + header_text + '" style="padding:4px!important;"></div>');
        th.html(input);
        if (bIsNumber)
        ...
        asInitVals[i] = label;
        var index = i;
        // as sTableId will be only one even in multiple table, create variable tid here to save for each table
        var tid = sTableId;

        if (bIsNumber && !oTable.fnSettings().oFeatures.bServerSide) {
            input.keyup(function () {
                /* Filter on the column all numbers that starts with the entered value */
                jquery_datatable_obj[tid].fnFilter('^' + this.value, index, true, false);
            });
        } else {
            input.keyup(function () {
                ...
                jquery_datatable_obj[tid].fnFilter(value.replace(/[,]/g, '|'), index, regex, smart);
            });
        }
    ...
    function fnCreateRangeInput() {
        ...
        var from = $('<div class="ui small filter input" style="width: 40%;"><input col="' + i + '" id="' + sFromId + '"type="text" placeholder="Min" style="padding:4px!important;"></div>');
        th.append(from);
        //th.append(_fnRangeLabelPart(1));
        var sToId = sTableId + 'range_to_' + i;
        //var to = $('<input type="text" class="number_range_filter" id="' + sToId + '" rel="' + i + '"/>');
        var to = $('<div class="ui small filter input" style="width: 40%;"><input col="' + i + '" id="' + sToId + '"type="text" placeholder="Max" style="padding:4px!important;border-top-left-radius:0px;border-bottom-left-radius:0px;"></div>');
        th.append(to);
        ...
        */
        $.fn.dataTableExt.afnFiltering.push(
	        function (oSettings, aData, iDataIndex) {
                // only filter on the operation table
                var tableSrc = $('table[name="' + tid + '"]')[0];
        ...
        $('#' + sFromId + ',#' + sToId, th).keyup(function () {
            ...
            //oTable.fnDraw();
            jquery_datatable_obj[tid].fnDraw();
    ...
    $.fn.columnFilter = function (options) {
        ...
        jquery_datatable_obj[sTableId] = oTable;
